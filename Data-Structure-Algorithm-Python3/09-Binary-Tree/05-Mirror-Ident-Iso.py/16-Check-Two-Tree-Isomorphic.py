...
class Tree:
	...
	def isIsomorphic(tree1,tree2):
		...
		if tree1.key==tree2.key: 
			...
			# CASE 5 : If Node1 LEFT == Node2 RIGHT and Node1 right == Node2 LEFT OR LEFT==LEFT 
			if (
				Tree.isIsomorphic(tree1.left,tree2.right) and Tree.isIsomorphic(tree1.right,tree2.left) 
														or
				Tree.isIsomorphic(tree1.left,tree2.left) and Tree.isIsomorphic(tree1.right,tree2.right) 
				):
					return True
			else: return False
...
result = Tree.isIsomorphic(tree1.root,tree2.root)
print(" Tree 1 and Tree2 are Isomorphic is  = ",result)
