...
# Root node is greates data Node in Heap and all the children are smaller than Parent
class BinaryMaxHeap:
	"""
	...
	def addNode(self,name,weight):
		...
		while parentIndex>=0:
			...
			if self.isSmaller(parentIndex,currentIndex):
	...
	def removeMax(self):
		...
		while leftChildIndex < self.size:
			...
			if rightChildIndex >=self.size:
				...
				if self.isSmaller(parentIndex,leftChildIndex):
					# swap the index of those key in positionMap
					self.positionMap.update({
			...
			if self.isSmaller(parentIndex,leftChildIndex) or self.isSmaller(parentIndex,rightChildIndex):

				# if left child is greater than right child 	
				if self.isSmaller(rightChildIndex,leftChildIndex):
	...
	def increaseNodeData(self,key,newData):
		...
		while parentIndex >= 0 :
			...
			if self.isSmaller(parentIndex,keyIndex):
				# swap the index of those key in positionMap
				self.positionMap.update({
