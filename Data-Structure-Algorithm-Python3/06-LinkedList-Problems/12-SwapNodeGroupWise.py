...
class LinkedList(object):
	...
	# swapNodes in group of k	
	def swapNodes(self,k=2):
		...
		current=self.head
		groupCount=0
		# Run until not reached to last node 			
		while current:	
			...
			# (Diff 1) newGroupStart Node . in next Iteration it will be helpful for
			# pointing to IT . 
			newGroupStart = current  # become End After reversal
			...
			# if group is reversed First group
			if groupCount==1:
				# assign new Start of List which is OldEnd or newStart of the First Group
				self.head=prev
			# if group is reversed
			else:
				# oldGroup_End_next will point to current Group Start 
				prevGroupEnd.next= prev

			# store newGroupStart for next iteration *** Diff(2)
