...
def rec_binary_search(arr,search_ele):

	if len(arr)==0: return False,None
	else:
		...
		if search_ele>arr[mid]:
			return rec_binary_search(arr[mid+1:],search_ele)
		
		# search element is greater than mid then search toward Right side <----
		else:
			return rec_binary_search(arr[:mid],search_ele)
...
if __name__ == '__main__':
	...
	result=rec_binary_search(age,30)
	printResult(result)
	result=rec_binary_search(age,63)
	printResult(result)
	result=rec_binary_search(age,40)
	printResult(result)
