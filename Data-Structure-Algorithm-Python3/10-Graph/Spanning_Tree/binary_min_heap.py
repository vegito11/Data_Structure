...
class BinaryMinHeap:
	...
	def removeMin(self):
		...
		if self._isEmpty():	return False

		# 1) get first Node from Heap	
		minNode=self._allNodeList[0]

		# remove this first element from positionMap		
		self.positionMap.pop(minNode.getKey())
		...
		# if heap is _isEmpty then return without doing anything
		if self._isEmpty():	return minNode
	...
	def decreaseNodeData(self,key,newData):
		...
		# if newData is not smaller that current Content of this node return without doing anything
		if self._allNodeList[keyIndex].getData() <= newData: return

		# Modify the Data value of this Node
	...
	# O(1) operation because we have maintaiend Hash Table for this			
	def getNode(self,key):
		index=self.positionMap[key]
		return self._allNodeList[index].getData()
	# decrease the Node in the Heap	
